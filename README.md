...
## Project Structure
The repository is organized into C++ source files for the simulator, brain model, and rendering engine, plus Python export utilities under `nyx_ai/export/`.
...
On a Linux machine, you can produce a Windows `.exe`. The process requires a MinGW cross-compiler and building Bullet/GLFW from source for Windows.
...
2. Create the toolchain file `mingw64.cmake` (provided in repository root).

3. Build Bullet for Windows (static):
   ```bash
...
5. Integrate GLAD (replace `glcorearb.h` with GLAD) — see the comments in `render/Renderer.cpp`.

6. Cross-compile Abel:
   ```bash
   cd ~/abel
...
7. Deploy — copy `abel.exe`, `assets/`, and required DLLs (if any) to Windows.
...
## Online Learning
The OnlineTrainer uses the ExperienceBuffer to train the WorldDecoder policy using REINFORCE. Every time the AI takes a creation action, it logs the state, action, and reward. When the buffer is full, a training step is performed. This makes the AI’s creations progressively more interesting and surprising over time.

---

## Exporting Models
The Python package `nyx_ai/export/` (not compiled with Abel, but included in the repository) can convert trained transformer weights to GGUF, SafeTensors, and ONNX formats. These exported models can then be loaded by the C++ transformer runtime (future integration).
...
| No display | Set `DISPLAY=:0` if you have a desktop, or use `xvfb-run`. |
...
The GGML transformer implementation derives from `llama.cpp`. The rendering engine uses OpenGL, GLFW, and Bullet Physics.
...
For questions, contributions, or to share your Abel-generated worlds, visit [https://github.com/hackermalade/abel](https://github.com/hackermalade/abel).
